...
export function CustomExpandedComponent<TData extends Record<string, any>>(row: Row<TData>) {
  ...
  if (!Array.isArray(products) || products.length === 0) {
    return (
      <Flex
        ...
        justify="center"
      >
        <Text className="p-4 text-2xl text-gray-500">No products available for this order.</Text>
      </Flex>
    );
  }
        ...
        >
          <div className="flex items-start">
            ...
            <header>
              ...
              <Text className="text-xs text-gray-500">Unit Price: ${product.price}</Text>
            </header>
          </div>
